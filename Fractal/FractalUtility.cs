...
namespace Fractal
{
    public class FractalUtility
    {
        ...
        /// <summary>
        /// Returns the cutoff levels corresponding to regions assigned according to the level weights.
        /// </summary>
        /// <param name="map">The fractal</param>
        /// <param name="levels">Weights of the different regions</param>
        /// <returns></returns>
        public static List<float> getCutOffLevels(float[,] map, float[] levels)
        {
            ...
            {
                t += levels[i];
                cutOffLevels.Add(arr[(int)Math.Floor(arr.Length * t / levelSum)]);
        ...
        /*public static float[,] fractal(int n, int m, FractalOptions options){
            ...
            int size = (int)Math.Floor((double)Math.Max(n, m));
        ...
        {
            var l = Math.Ceiling(Math.Log(x) / Math.Log(2));
            return (int)Math.Round(Math.Pow(2, l));
